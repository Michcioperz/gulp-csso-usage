...
module.exports = new Transform({objectMode: true, transform: function(file, encoding, callback) {
  ...
  let check_tag = function (tag) {
    if (tag.type == 'tag') {
      ...
      if (tag.attribs) {
        ...
        if (tag.attribs['id'])
          usage.ids.add(tag.attribs['id'].trim());
      }
    }
  };
  ...
  for (let tag of htmlp._handler.dom)
      check_tag(tag);
  file.path = gutil.replaceExtension(file.path, '.json');
  file.contents =  new Buffer(JSON.stringify({tags: Array.from(usage.tags), classes: Array.from(usage.classes), ids: Array.from(usage.ids)}));
